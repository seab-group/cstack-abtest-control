...
**Screenshot evidence rule (mandatory for UI tasks):**
For every `human-verify` AC that targets a web UI, you MUST take a screenshot and record it in the evidence table. Steps:
1. Navigate to the relevant state using `${BROWSE_BIN:-gstack} goto $QA_BASE_URL` and any needed interactions.
2. Capture: `${BROWSE_BIN:-gstack} screenshot /tmp/qa-<task-id>-ac<N>-$(date +%s).png`
3. Record in evidence table: screenshot path, the `file:line` code reference that implements the behaviour, and a one-sentence "observed vs expected" note.

A QA pass with no screenshots for a UI task is treated as an incomplete run and will be reverted. If `gstack` is not on PATH, exit with `env_error` and mailbox `tshepo.md`: "gstack browse not found — install gstack to enable screenshot evidence".

**qa-smoke.sh — required for console UI tasks:**
When the task spec mentions `QA_BASE_URL` or a web endpoint AND the task has one or more `human-verify` ACs, you MUST run the Fleet Console smoke script before declaring a pass:

```bash
bash supervisor/console/qa-smoke.sh
```

The script navigates to `$QA_BASE_URL` (default `http://localhost:7842`), asserts the page title, tab bar, and Fleet tab are present, and writes a screenshot to `/tmp/console-qa-<timestamp>.png`. Copy the printed `Screenshot:` path into your `task complete --verdict passed` comment as evidence. Do not rely solely on `curl` against JSON API endpoints to verify UI tasks — the smoke script provides the browser-level confirmation that the UI is actually rendering.
...
|---|---|---|---|
| AC1 | /tmp/qa-<task-id>-ac1-<ts>.png | <file>:<line> — <symbol or behaviour> | <what you saw> ✓ |
| AC2 | /tmp/qa-<task-id>-ac2-<ts>.png | <file>:<line> — <symbol or behaviour> | <what you saw> ✓ |
```
Every `human-verify` AC targeting a UI must have a row. ACs of type `done_check`/`e2e_check` record pass/fail only — no screenshot required.
